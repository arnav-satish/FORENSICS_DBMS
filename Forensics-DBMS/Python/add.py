...
def add():
    menu = ["Automobile", "Ballistics", "Cases", "Criminal", "Drugs","Paint", "CriminalCase", 'User_Credentials']
    ...
    #Add more user or admins
    if choice == menu[7]:   
        col1, col2 = st.columns(2)
        with col1:
            id = st.text_input("Username")
            lo = st.text_input("Password")
        with col2:
            name = st.selectbox("Role", ["user", "admin"])
        if st.button("Add User"):
            try:
                add_user(id, lo, name)
                st.success("Successfully added user: {}".format(id))
